...
async def callback_deposit_init(callback: types.CallbackQuery, config: BotConfig, state: FSMContext, user_model: ModelUser, api_client: OTPAPIClient) -> None:
    ...
    action_model = ModelAction(current_action="deposit", state=state, chat_id=callback.message.chat.id)
    ...
    answer = await callback.message.answer("Silahkan ketikkan jumlah deposit, contoh: 10000")
...
async def ask_payment_method(amount: str, state: FSMContext, user_model: ModelUser, action_model: ModelAction, callback: types.CallbackQuery = None) -> None:
    ...
    if amount is None or amount == "":
        user_model.add_message_id((await bot.send_message(chat_id=action_model.chat_id, text="Silahkan ketikkan jumlah deposit, contoh: 10000")).message_id)
